...
jQuery.extend({
  ...
  css: function (e, p) {
    ...
    } else if (p == 'opacity' && jQuery.browser.msie) { return parseFloat(jQuery.curCSS(e, 'filter').replace(/[^0-9.]/, '')) || 1 }
  ...
  curCSS: function (elem, prop, force) {
    ...
    if (!force && elem.style[prop]) {
      ...
      if (cur) { ret = cur.getPropertyValue(prop) } else if (prop == 'display') { ret = 'none' } else {
  ...
  clean: function (a) {
    ...
    for (var i = 0; i < a.length; i++) {
      ...
      } else if (a[i] !== null) { r.push(a[i].nodeType ? a[i] : document.createTextNode(a[i].toString())) }
    }
    return r
  },
  ...
  token: [
    ...
    '~', function (a) {
      ...
      if (s.n > 0) {
        for (var i = s.n; i < s.length; i++) { r.push(s[i]) }
      }
      return r
    }
  ],
    ...
    remove: function (element, type, handler) {
      if (element.events) {
        if (type && element.events[type]) {
          if (handler) { delete element.events[type][handler.guid] } else {
            for (var i in element.events[type]) { delete element.events[type][i] }
          }
        } else {
          for (var j in element.events) { this.remove(element, j) }
        }
      }
    },
...
jQuery.macros = {
  ...
  each: {
    ...
    remove: function (a) {
      if (!a || jQuery.filter([this], a).r) { this.parentNode.removeChild(this) }
    },
    empty: function () {
      while (this.firstChild) { this.removeChild(this.firstChild) }
    },
    bind: function (type, fn) {
      if (fn.constructor == String) { fn = new Function('e', (!fn.indexOf('.') ? '$(this)' : 'return ') + fn) }
...
jQuery.extend({
  ...
  ready: function () {
    ...
    if (!jQuery.isReady) {
      ...
      if (jQuery.readyList) {
        // Execute all of them
        for (var i = 0; i < jQuery.readyList.length; i++) { jQuery.readyList[i].apply(document) }
  ...
  animate: function (prop, speed, callback) {
    return this.queue(function () {
      ...
      for (var p in prop) {
        ...
        if (prop[p].constructor == Number) { e.custom(e.cur(), prop[p]) } else { e[ prop[p] ](prop) }
      }
    })
  },
...
jQuery.extend({
  ...
  speed: function (s, o) {
    ...
    o.complete = function () {
      ...
      if (o.oldComplete && o.oldComplete.constructor == Function) { o.oldComplete.apply(this) }
    }
  ...
  fx: function (elem, options, prop) {
    ...
    z.a = function () {
      ...
      if (prop == 'opacity') {
        ...
        if (window.ActiveXObject) { y.filter = 'alpha(opacity=' + z.now * 100 + ')' } else { y.opacity = z.now }
    ...
    z.step = function (firstNum, lastNum) {
      ...
      if (t > z.o.duration + z.startTime) {
        ...
        for (var i in z.el.curAnim) {
          if (z.el.curAnim[i] !== true) { done = false }
        }
